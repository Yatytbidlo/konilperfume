...
def callback_query(call):
    ...
    if call.data == "back":
        ...
        user_state.append("register")
        set_user_state(call.from_user.id, user_state)
        msg = bot.send_message(call.from_user.id, "Пожалуйста, введите ваш номер телефона:")
        ...
        if product_info:
            ...
            buttons_info.add(InlineKeyboardButton("Заказать", callback_data="register"))

            if product_info['image'] and is_url_accessible(product_info['image']):
                bot.send_photo(call.from_user.id, photo=product_info['image'], caption=info_text, reply_markup=buttons_info, parse_mode='Markdown')
            else:
...
# Функция для обработки шага ввода телефона
def process_phone_step(message):
    phone = re.sub(r'\D', '', message.text)  # Очистка номера телефона, оставляем только цифры
    if not phone.isdigit():
        msg = bot.reply_to(message, 'Пожалуйста, введите корректный номер телефона.')
        bot.register_next_step_handler(msg, process_phone_step)
        return

    existing_user = check_existing_user(phone)
    if existing_user:
        payment_button = InlineKeyboardMarkup()
        payment_button.add(InlineKeyboardButton("Оплатить через Kaspi QR", url="https://pay.kaspi.kz/pay/pcyzkuxw"))
        payment_button.add(InlineKeyboardButton("Написать в WhatsApp", url="wa.me/+77076017555 "))
        bot.send_message(
            message.chat.id, 
            f"**Добро пожаловать обратно, {existing_user['name']}!**\n\n"
            "Для завершения заказа нажмите кнопку **Оплатить через Kaspi QR**, если вы оплачиваете через Kaspi QR, после оплаты отправьте чек на наш WhatsApp, нажав на кнопку ниже, и наш менеджер свяжется с вами.\n"
            "Если у вас другой способ оплаты, просим нажать на кнопку WhatsApp для уточнения деталей.", 
            reply_markup=payment_button, parse_mode='Markdown'
        )
        set_user_state(message.chat.id, [])  # Сброс состояния пользователя
        return  # Завершаем процесс регистрации для существующего пользователя
    else:
        user_state = get_user_state(message.chat.id)
        user_state.append(('phone', phone))
        set_user_state(message.chat.id, user_state)
    
    msg = bot.send_message(message.chat.id, 'Введите ваше имя:')
    bot.register_next_step_handler(msg, process_name_step)

# Функция для обработки шага ввода имени
def process_name_step(message):
    name = message.text
    user_state = get_user_state(message.chat.id)
    user_state.append(('name', name))
    set_user_state(message.chat.id, user_state)
    
    msg = bot.send_message(message.chat.id, 'Введите ваш адрес:')
    bot.register_next_step_handler(msg, process_address_step)

# Функция для обработки шага ввода адреса
def process_address_step(message):
    address = message.text
    user_state = get_user_state(message.chat.id)
    user_state.append(('address', address))
    set_user_state(message.chat.id, user_state)
    
    phone = next((item[1] for item in user_state if item[0] == 'phone'), None)
    name = next((item[1] for item in user_state if item[0] == 'name'), None)
    address = next((item[1] for item in user_state if item[0] == 'address'), None)
    
    try:
        query = "INSERT INTO users (phone, name, address) VALUES (%s, %s, %s)"
        cursor.execute(query, (phone, name, address))
        conn.commit()
        payment_button = InlineKeyboardMarkup()
        payment_button.add(InlineKeyboardButton("Оплатить через Kaspi QR", url="https://pay.kaspi.kz/pay/pcyzkuxw"))
        payment_button.add(InlineKeyboardButton("Написать в WhatsApp", url="wa.me/+77076017555 "))
        ...
        print(f"Ошибка выполнения запроса: {err}")
        bot.send_message(message.chat.id, "Ошибка регистрации. Пожалуйста, попробуйте еще раз.")

    set_user_state(message.chat.id, [])
